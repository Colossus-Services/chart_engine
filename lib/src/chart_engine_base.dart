...
abstract class ChartEngine {
  ...
  void checkLoaded() {
    if ( !isLoaded ) throw StateError('Trying to render before load() engine[${ runtimeType }]!') ;
  }

  /// Renders at [output] a [chartSeries], selecting the correct render method.
  bool render(Element output, ChartData chartSeries) {
    if (chartSeries == null) return false;

    checkLoaded();

    if (chartSeries is ChartSeries) {
      return renderLineChart(output, chartSeries);
    } else if (chartSeries is ChartSet) {
      return renderBarChart(output, chartSeries);
    }
...
class ChartEngineSwitchable extends ChartEngine {
  ...
  ChartEngineSwitchable(this.engines, { Type mainEngineType , ChartEngine mainEngine }) {
  ...
  @override
  bool get isLoaded => listMatchesAll( engines.map( (e) => e.isLoaded ) , (bool r) => r ) ;
